...
async fn main() -> std::io::Result<()> {
    ...
    register_decoding_pipelines(config.clone(), &mut pipelines, receiver).await;
    register_encoding_pipelines(config.clone(), &mut pipelines, sender).await;
...
async fn register_decoding_pipelines<T: FrameProcessor + Send + 'static>(
    ...
    register!(
        ...
        "decoding",
        AscodePipeline::new()
            .link(Component::new().append(receivers::generic(&mut decode_pools, receive_processor)))
            .link(
                Component::new()
...
async fn register_encoding_pipelines<T: FrameProcessor + Send + 'static>(
    config: Configuration,
    pipelines: &mut PipelineRegistry,
    send_processor: T,
) {
        ...
        AscodePipeline::new()
            ...
            .link(
                Component::new()
                    ...
                    .append(senders::generic(&mut encode_pools, send_processor))
            )
    );
}
